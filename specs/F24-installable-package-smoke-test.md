...
**Scope (locked):**
1. **Fresh venv installation test** — Create a fresh venv, install the package from the local repo path (as proxy for `pip install git+https://github.com/RonKG/docling_spatial_pdfs.git`), and verify the install succeeds with all dependencies resolved (`docling`, `docling-core`, `openai`, `pydantic`, `jsonschema`).
